...
/*  Item *items, st;
  ...
  //st = omp_get_wtime();
  Item *items = read_csv(filename, number_of_items);
  //printf("Reading time: %f\n", omp_get_wtime() - st);

  *elements_per_item = data[0].len;
  size_t size = *elements_per_item * sizeof(double);
  items = (Item*)malloc(*number_of_items * sizeof(Item));

  for (i = 0; i < *number_of_items; i++)
  {
    items[i].value = data[i].numeric;//(double*)malloc(size);
    for (j = 0; j < *elements_per_item; j++)
      items[i].value[j] = data[i].numeric[j];
  }
